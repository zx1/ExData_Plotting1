...
Plot4 = function(){
	...
	# plot in a 2x2 square 4 plots that include line charts and series charts
	# save the result to PNG
    png(file="plot4.png", width=480, height=480)
    par(mfcol=c(2,2))
    
    plot(x=sdt, y=data$Global_active_power, type="l", ylab="Global Active Power", xlab="")
    ...
    plot(x=sdt, y=data$Voltage, type="l", ylab="Voltage", xlab="datetime")
    axis.POSIXct(1,at = seq(StartDateTime, StopDateTime, "days"), format = "%a")

    plot(x=sdt, y=data$Global_reactive_power, type="l", ylab="Global_reactive_power", xlab="datetime")
    axis.POSIXct(1,at = seq(StartDateTime, StopDateTime, "days"), format = "%a")
